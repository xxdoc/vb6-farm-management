...
Begin VB.Form frmCapitalMovement 
   BackColor       =   &H80000000&
   ...
   ClientTop       =   330
   ClientWidth     =   10410
   Icon            =   "frmCapitalMovement.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MaxButton       =   0   'False
   ...
   Begin Threed.SSFrame SSFrame1 
      ...
      Begin Threed.SSCommand cmdStart 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmCapitalMovement.frx":27A2
         ButtonStyle     =   3
      End
      ...
      Begin Threed.SSCommand cmdOK 
         ...
         _Version        =   131073
         MousePointer    =   99
         MouseIcon       =   "frmCapitalMovement.frx":2ABC
         ButtonStyle     =   3
      End
   End
End
Attribute VB_Name = "frmCapitalMovement"
...
'Call S.PrintDebug(Cm.DOCUMENT_NO, "GenerateExportMoveMent1", EI.EXPORT_AMOUNT, EI.TX_TYPE)
   Else
      TempSearch.CAPITAL_AMOUNT = TempSearch.CAPITAL_AMOUNT + S.CAPITAL_AMOUNT
'Call TempSearch.PrintDebug(Cm.DOCUMENT_NO, "GenerateExportMoveMent2", EI.EXPORT_AMOUNT, EI.TX_TYPE)
   End If

'If (EI.PIG_ID & "-" & EI.HOUSE_ID = "9341-278") Then
'glbErrorLog.LocalErrorMsg = EI.DOCUMENT_NO & ", PART_ITEM_ID=" & S.PART_ITEM_ID & ", " & "CAPITAL_AMOUNT=" & S.CAPITAL_AMOUNT
...
Private Sub GeneratePigImportMoveMent(II As CImportItem, BatchID As Long)
Dim Cm As CCapitalMovement
Dim Mi As CMovementItem
Dim S As CMovementItemSearch1
Dim TempSearch As CMovementItemSearch1
...
Static TempCol As Collection
   ...
   If (FirstDate <> TempFirstDate) Or (LastDate <> TempLastDate) Then
      ...
      Call LoadPigBirthAmount(Nothing, TempCol, FirstDate, LastDate, CommitTypeToFlag(cboCommitType.ItemData(Minus2Zero(cboCommitType.ListIndex))))
...
Private Sub GeneratePigBirthMoveMent(II As CImportItem, BatchID As Long)
Dim Cm As CCapitalMovement
Dim Mi As CMovementItem
...
Dim iCount As Long
   ...
   If (TempFirstDateStr <> FirstDateStr) Or (TempLastDateStr <> LastDateStr) Then
      ...
      Set TempCol = Nothing
      Set TempCol = New Collection
      Call LoadPigParentUseAmount(Nothing, TempCol, FirstDate, LastDate, CommitTypeToFlag(cboCommitType.ItemData(Minus2Zero(cboCommitType.ListIndex))))
   End If
   ...
   If TempSearch Is Nothing Then
      ...
      For Each EI In TempCol
         ...
         If TempSearch Is Nothing Then
            Call m_MovementItemSearchs1.Add(S, S.GetKey1)
'Call S.PrintDebug(Cm.DOCUMENT_NO, "GeneratePigBirthferMovement", II.IMPORT_AMOUNT, II.TX_TYPE)
         Else
            TempSearch.CAPITAL_AMOUNT = TempSearch.CAPITAL_AMOUNT + S.CAPITAL_AMOUNT
'Call TempSearch.PrintDebug(Cm.DOCUMENT_NO, "GeneratePigBirthferMovement", II.IMPORT_AMOUNT, II.TX_TYPE)
         End If
      Next EI
   End If
...
Dim PigParentInHouse1 As Long
   ...
   If Not m_Rs.EOF Then
      '26-06-2555
      'กรณีที่มีใบเกิดอยู่แล้ว เดิมไม่ต้องทำอะไรเลยแต่ตอนนี้จะ Set ค่า II.IMPORT_AMOUNT = 0 เพื่อให้ให้เบ้น ประกอบกับ SET ให้ II.IMPORT_AMOUNT =PigBirthInMonthInHouse เลย
      'เพื่อแก้ปัญหาเวลาเกิดปันต้นทุนแค่ใบเดียวแต่ ยอด m_MovementItemSearchs3 มันเพิ่มขึ้นแค่ ยอดใบเกิดนั้นๆ
      II.IMPORT_AMOUNT = 0
      Set TempII = Nothing
      Exit Sub
   End If
   ...
   If TempSearch Is Nothing Then
      ...
      For Each EI In TempCol
         ...
         Set S = Nothing
      Next EI
      II.IMPORT_AMOUNT = PigBirthInMonthInHouse
   Else
   '26-06-2555
   'กรณีที่มีใบเกิดอยู่แล้ว เดิมไม่ต้องทำอะไรเลยแต่ตอนนี้จะ Set ค่า II.IMPORT_AMOUNT = 0 เพื่อให้ให้เบ้น ประกอบกับ SET ให้ II.IMPORT_AMOUNT =PigBirthInMonthInHouse เลย
   'เพื่อแก้ปัญหาเวลาเกิดปันต้นทุนแค่ใบเดียวแต่ ยอด m_MovementItemSearchs3 มันเพิ่มขึ้นแค่ ยอดใบเกิดนั้นๆ
...
Private Sub GenerateExpenseMovement(Ri As CROItem, BatchID As Long)
...
Dim PigImportAmountsBal As Collection
Dim PigExportAmountsBal As Collection
Dim PigExportAmountsBalEx As Collection
Dim PigExportAmountsBalEx1 As Collection
Dim ImportItem As CImportItem
Dim BalAmount As Double
Dim NewDate As Date
Dim BalEi As CExportItem
Dim BalIi As CImportItem
Dim PigBirthInMonthInHouse As Long
   ...
   Set PigImportAmountsBal = New Collection
   Set PigExportAmountsBal = New Collection
   Set PigExportAmountsBalEx = New Collection
   Set PigExportAmountsBalEx1 = New Collection
   
   Call GetFirstLastDate(Ri.DOCUMENT_DATE, FirstDate, LastDate)
   
   Call LoadExpenseRatio(Nothing, ExpenseRatios, Ri.RO_ITEM_ID, BatchID)
   For Each Er In ExpenseRatios
      
      If Er.SELL_PARENT_FLAG = "Y" Then
         'เวลาขายพ่อแม่จะต้องนำค่าเสื่อมพ่อแม่หรือ EOF มาใส่ด้วย   จะ Set เดือนละครั้ง
         Call LoadPigExportAmount(Nothing, PigExportAmountsBalEx, FirstDate, LastDate, CommitTypeToFlag(cboCommitType.ItemData(Minus2Zero(cboCommitType.ListIndex))), Er.LOCATION_ID, , "Y", , , , BatchID, "(10,13)")
         ExpenseAmt = (Er.RATIO / 100) * Ri.TOTAL_PRICE
         PigInAllHouse = 0
         DoEvents
         For Each BalEi In PigExportAmountsBalEx
            'ยอดขายของหมู GLB ตามสัปดาห์เกิด
            PigInHouse = BalEi.EXPORT_AMOUNT
            'ยอดขายของหมู GLB ทุกสัปดาห์เกิด
         ...
         For Each BalEi In PigExportAmountsBalEx
            ...
            Cm.TX_AMOUNT = 0
            Cm.TO_PIG_COUNT = PigInHouse
            Cm.FROM_HOUSE_ID = Er.LOCATION_ID
            Cm.TO_HOUSE_ID = 0
            Cm.PIG_ID = BalEi.PART_ITEM_ID
            Cm.PIG_STATUS = 0
            Cm.TX_SEQ = Ri.TRANSACTION_SEQ
            Cm.REPLACE_FLAG = "N"
            Cm.BATCH_ID = BatchID
            Call Cm.AddEditData
            ...
            If PigInHouse > 0 Then
               ...
               Mi.CAPITAL_AMOUNT = MyDiffEx(ExpenseAmt, PigInAllHouse) * PigInHouse
               'Mi.CAPITAL_AMOUNT = MyDiffEx(Ri.TOTAL_PRICE, PigInAllHouse) * PigInHouse
      ...
      Else
         NewDate = InternalDateToDate(DateToStringIntHi(DateAdd("D", -1, Ri.DOCUMENT_DATE)))
         Call LoadPigImportAmount(Nothing, PigImportAmountsBal, -1, NewDate, CommitTypeToFlag(cboCommitType.ItemData(Minus2Zero(cboCommitType.ListIndex))), Er.LOCATION_ID, , "N", , , BatchID)
         Call LoadPigExportAmount(Nothing, PigExportAmountsBal, -1, NewDate, CommitTypeToFlag(cboCommitType.ItemData(Minus2Zero(cboCommitType.ListIndex))), Er.LOCATION_ID, , "N", , , , BatchID)
         
         Call LoadImportPig(Nothing, PigInHouses, Er.LOCATION_ID, , BatchID)
         ExpenseAmt = (Er.RATIO / 100) * Ri.TOTAL_PRICE
         PigInAllHouse = 0
         DoEvents
         
         For Each II In PigInHouses
            Set BalIi = GetImportItem(PigImportAmountsBal, Trim(Str(II.PART_ITEM_ID)))
            Set BalEi = GetExportItem(PigExportAmountsBal, Trim(Str(II.PART_ITEM_ID)))
            
            'ยอดยกมาของหมูที่ไม่ใช่ GLB + หมูเกิด
            PigBirthInMonthInHouse = GetPigBirthInMonthInHouse(FirstDate, LastDate, Er.LOCATION_ID, II.PART_ITEM_ID, BatchID)
            BalAmount = BalIi.IMPORT_AMOUNT - BalEi.EXPORT_AMOUNT
            If BalAmount >= 0 Then
               PigInHouse = BalAmount + PigBirthInMonthInHouse
               'ยอดยกมาของหมูที่ไม่ใช่ GLB รวมทุกสัปดาห์ในโรงเรือน
               PigInAllHouse = PigInAllHouse + PigInHouse
            End If
         Next II
   ...
   Set PigImportAmountsBal = Nothing
   Set PigExportAmountsBal = Nothing
   Set Mi = Nothing
   Set Cm = Nothing
End Sub
...
Dim iCount As Long
   ...
   For Each II In m_MovementItemSearchs3
      If II.LOCATION_ID = 446 And II.PART_ITEM_ID = 13749 Then
         ''debug.print II.CURRENT_AMOUNT
      End If
   Next II
End Function

Private Sub GeneratePigTransferMovementExp(EI As CExportItem, II As CImportItem, BatchID As Long)
...
'
'If EI.DOCUMENT_NO = "H562/28083" Then
'''debug.print
'End If
'If (EI.DOCUMENT_NO = "H576/28763") And (EI.PART_ITEM_ID & "-" & EI.LOCATION_ID = "9341-278") Then
'''debug.print
'End If
...
'If (EI.PART_ITEM_ID & "-" & EI.LOCATION_ID = "9341-278") Then
'glbErrorLog.LocalErrorMsg = EI.DOCUMENT_NO & ", PART_ITEM_ID=" & S.PART_ITEM_ID & ", " & "CAPITAL_AMOUNT=" & Mi.CAPITAL_AMOUNT
'glbErrorLog.ShowUserErrorEx
'TempAmt = TempAmt + Amt
'End If
...
Private Sub GeneratePigTransferMovementImp(II As CImportItem, FromEi As CExportItem, BatchID As Long)
Dim Cm As CCapitalMovement
Dim Mi As CMovementItem
Dim iCount As Long
Dim TempRs As ADODB.Recordset
...
Dim Lc As CLocation
   ...
   If O.TX_TYPE = "I" Then
      ...
      'Set Pi = GetPartItem(m_PartItems, Ii.PART_ITEM_ID)
      Set Lc = GetLocation(m_Locations, II.LOCATION_ID)
      Amt = II.IMPORT_AMOUNT
   ElseIf O.TX_TYPE = "E" Then
      Set EI = O
      Key = EI.PART_ITEM_ID & "-" & EI.LOCATION_ID
      'Set Pi = GetPartItem(m_PartItems, Ei.PART_ITEM_ID)
      Set Lc = GetLocation(m_Locations, EI.LOCATION_ID)
   ...
   If TempII Is Nothing Then
      Set TempII = New CImportItem
      'TempII.PART_DESC = Pi.PART_DESC
      'TempII.PART_NO = Pi.PART_NO
      'TempII.LOCATION_NAME = Lc.LOCATION_NAME
...
'If (O.PART_ITEM_ID = 8897) And (O.LOCATION_ID = 255) Then
'glbErrorLog.LocalErrorMsg = "Now ...'" & O.DOCUMENT_NO & "' AMT = " & Amt & "  CURRENT AMOUNT = " & TempII.CURRENT_AMOUNT
'glbErrorLog.ShowUserErrorEx
'End If

   Set Ba = Nothing
   
   ' สำหรับ ทดสอบ ว่ามี การเพิ่ม อยู่ที่ลำดับ เท่าไหร่
'   iCount = 0
'   For Each Ii In m_MovementItemSearchs3
'      iCount = iCount + 1
'      If Ii.PART_ITEM_ID = 13862 And Ii.LOCATION_ID = 504 Then
'      'If Ii.LOCATION_ID = 504 Then
...
'Dim TempBirth As Long
   ...
   If uctlFromDate.ShowDate > 0 Then
      Call LoadInitialCapitalBalance1(Nothing, m_MovementItemSearchs1, -1, NewDate, , , , BatchID)
   ...
   Call LoadRelatedImportItem(Nothing, m_ImportItems, uctlFromDate.ShowDate, uctlToDate.ShowDate, "", 1, BatchID)
   Call LoadRelatedExportItem(Nothing, m_ExportItems, uctlFromDate.ShowDate, uctlToDate.ShowDate, "", 1, BatchID)
   ...
   While (I = 0) Or (Not (O Is Nothing))
      ...
      Set O = glbDaily.QueryAllTransaction(Ivd, IsOK, iCount, glbErrorLog, , BatchID)
      
'      If O.LOCATION_ID = 504 And (O.PART_ITEM_ID = 13862) Then
'         'And O.PART_ITEM_ID <= 13853
'         ''debug.print (O.PART_DESC)
'      End If
      
'      If O.DOCUMENT_NO = "E098/04887" Then
'        ''debug.print (O.DOCUMENT_NO & "-" & O.LOCATION_ID & "-" & O.PART_ITEM_ID)
'      End If
      
'      For Each II In m_MovementItemSearchs3
'         If II.LOCATION_ID = 542 And II.PART_ITEM_ID = 14577 Then
'            ''debug.print II.CURRENT_AMOUNT
'         End If
'      Next II
      
      'And O.LOCATION_ID = 446 And O.PART_ITEM_ID = 13749 Or O.DOCUMENT_NO = "ปันค่าใช้จ่าย 5-55"
      
      If Not (O Is Nothing) Then
         '''debug.print (O.DOCUMENT_NO & "-" & O.DOCUMENT_DATE & "-" & O.LOCATION_ID & "-" & O.PART_ITEM_ID)
         '''debug.print (O.DOCUMENT_NO & "-" & O.DOCUMENT_DATE & "-" & O.LOCATION_ID & "-" & O.PART_ITEM_ID & "-" & O.HOUSE_ID & "-" & O.PIG_ID)
         If (O.TX_TYPE = "E") And (O.DOCUMENT_TYPE = 2) Then 'เบิกวัตถุดิบ
            Call GenerateExportMoveMent(O, BatchID)
         ElseIf (O.TX_TYPE = "I") And (O.DOCUMENT_TYPE = 11) Then 'ใบซื้อสุกร
            Call GeneratePigImportMoveMent(O, BatchID)
         ...
         ElseIf (O.TX_TYPE = "I") And ((O.DOCUMENT_TYPE = 6) Or (O.DOCUMENT_TYPE = 7) Or (O.DOCUMENT_TYPE = 8)) Then  'ใบโอนสุกรเข้าเรือนขาย
            ...
            Call GeneratePigTransferMovementImp(O, EI, BatchID)
            Call UpdateCurrentAmount(O)
            Set EI = Nothing
         ElseIf (O.TX_TYPE = "E") And ((O.DOCUMENT_TYPE = 6) Or (O.DOCUMENT_TYPE = 7) Or (O.DOCUMENT_TYPE = 8) Or (O.DOCUMENT_TYPE = 9) Or (O.DOCUMENT_TYPE = 10) Or (O.DOCUMENT_TYPE = 13)) Then 'ใบโอนสุกรเข้าเรือนขาย
            ...
            Call GeneratePigTransferMovementExp(O, II, BatchID)
            Call UpdateCurrentAmount(O)
            Set II = Nothing
         ElseIf (O.TX_TYPE = "I") And (O.DOCUMENT_TYPE = 5) Then 'ใบสุกรคลอด
            Call GeneratePigBirthMoveMentEx(O, BatchID)
            Call UpdateCurrentAmount(O)
            'TempBirth = TempBirth + O.IMPORT_AMOUNT
         ElseIf (O.TX_TYPE = "X") And (O.DOCUMENT_TYPE = 5) Then 'ใบรับของ ระบบงานซื้อ
            Call GenerateExpenseMovement(O, BatchID)
...
'      If Ii.LOCATION_ID = 504 And (Ii.CURRENT_AMOUNT <= 0) And Ii.PART_ITEM_ID = 13862 Then
'         ''debug.print
'      End If
      ...
      If (II.CURRENT_AMOUNT <= 0) And (II.LOCATION_ID > 0) And (II.PART_ITEM_ID > 0) Then
         ...
         For Each S In m_MovementItemSearchs1 'วัตถุดิบ
            If (S.PIG_ID = II.PART_ITEM_ID) And (S.HOUSE_ID = II.LOCATION_ID) And S.CAPITAL_AMOUNT <> 0 Then 'II.PART_ITEM_ID is PIG_ID
               '''debug.print Ii.LOCATION_ID & "-" & Ii.PART_NO & " ---> " & Ii.CURRENT_AMOUNT & " -(((( " & S.CAPITAL_AMOUNT
               Set Mi = New CMovementItem
               Mi.CAPITAL_MOVEMENT_ID = Cm.CAPITAL_MOVEMENT_ID
               Mi.AddEditMode = SHOW_ADD
         ...
         For Each S In m_MovementItemSearchs2 'ค่าใช้จ่าย
            If (S.PIG_ID = II.PART_ITEM_ID) And (S.HOUSE_ID = II.LOCATION_ID) And S.CAPITAL_AMOUNT <> 0 Then 'II.PART_ITEM_ID is PIG_ID
               '''debug.print Ii.LOCATION_ID & "-" & Ii.PART_NO & " ---> " & S.CAPITAL_AMOUNT
               Set Mi = New CMovementItem
               Mi.CAPITAL_MOVEMENT_ID = Cm.CAPITAL_MOVEMENT_ID
               Mi.AddEditMode = SHOW_ADD
               Mi.CAPITAL_AMOUNT = -1 * S.CAPITAL_AMOUNT
