...
Dim IsOK As Boolean
   ...
   If Not (DocumentType = POST_CHEQUE) Then
...
Dim I As Long
   ...
   ID = ConvertDocToConfigNo(2, DocumentType, DocumentSubType)
   If ID > 0 Then
      ...
      If Not (Cd Is Nothing) Then
         ...
         Call m_CashDoc.SetFieldValue("RUNNING_NO", Cd.GetFieldValue("RUNNING_NO") + 1 + DocAdd)
         Call m_CashDoc.SetFieldValue("CONFIG_DOC_TYPE", ID)
      Else
         txtDocumentNo.Text = ""
      End If
...
'         End If
      ElseIf (DocumentType = POST_CHEQUE) Then
         glbErrorLog.LocalErrorMsg = "ไม่สามารถที่จะเปิดมาแก้ไขได้กรณีลบแล้วสร้างใหม่"
         glbErrorLog.ShowUserError
         Exit Sub
      ElseIf (DocumentType = CASH_DEPOSIT) Then
...
Dim ReportMode As Long
   ...
   Set oMenu = New cPopupMenu
   lMenuChosen = oMenu.Popup("ใบสำคัญนำฝากเงิน จากเงินสด ไปยัง ธนาคาร", "ปรับค่าหน้ากระดาษ", "-", "ใบสำคัญนำฝากเงิน จากเงินสด ไปยัง เดินสะพัด", "ปรับค่าหน้ากระดาษ", "-", "ใบสำคัญนำฝากเงิน   จากเดินสะพัด ไปยัง ธนาคาร", "ปรับค่าหน้ากระดาษ")
   If lMenuChosen = 0 Then
      Exit Sub
   End If
   ...
   If lMenuChosen = 1 Then
      ReportKey = "CReportVocherRcp002"
      
      Set Report = New CReportVocherRcp002
      ReportFlag = True
   ...
   ElseIf lMenuChosen = 2 Then
      ...
      HeaderText = MapText("ใบสำคัญนำฝากเงิน จากเงินสด ไปยัง ธนาคาร")
   ...
   ElseIf lMenuChosen = 5 Then
      ...
      Rc.REPORT_KEY = ReportKey
      Call Rc.QueryData(m_Rs, iCount)
      HeaderText = MapText("ใบสำคัญนำฝากเงิน จากเงินสด ไปยัง เดินสะพัด")
      If Not m_Rs.EOF Then
         Call Rc.PopulateFromRS(1, m_Rs)
         EditMode = SHOW_EDIT
      Else
   ...
   ElseIf lMenuChosen = 8 Then
      ...
      Rc.REPORT_KEY = ReportKey
      Call Rc.QueryData(m_Rs, iCount)
      HeaderText = MapText("ใบสำคัญนำฝากเงิน   จากเดินสะพัด ไปยัง ธนาคาร")
      If Not m_Rs.EOF Then
         Call Rc.PopulateFromRS(1, m_Rs)
         EditMode = SHOW_EDIT
      Else
   ...
   If Not Report Is Nothing Then
      Call Report.AddParam(m_CashDoc.GetFieldValue("CASH_DOC_ID"), "CASH_DOC_ID")
