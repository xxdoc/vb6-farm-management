...
'   End If
   ...
   m_HouseGroup.AddEditMode = ShowMode
   m_HouseGroup.HOUSE_GROUP_ID = ID
   m_HouseGroup.HOUSE_GROUP_NO = txtDocumentNo.Text
   m_HouseGroup.HOUSE_GROUP_NAME = txtMotherNo.Text
   m_HouseGroup.EXTRA_FLAG = Check2Flag(chkExtraFlag.Value)
   Call PopulateItem(m_HouseGroup.HGroupItems)
   
   Call EnableForm(Me, False)
   If Not glbMaster.AddEditHouseGroup(m_HouseGroup, IsOK, glbErrorLog) Then
      glbErrorLog.ShowErrorLog (LOG_FILE_MSGBOX)
      SaveData = False
      Call EnableForm(Me, True)
      Exit Function
...
Private Sub Form_Activate()
   If Not m_HasActivate Then
      ...
      If (ShowMode = SHOW_EDIT) Or (ShowMode = SHOW_VIEW_ONLY) Then
         m_HouseGroup.QueryFlag = 1
         Call QueryData(True)
      ElseIf ShowMode = SHOW_ADD Then
         m_HouseGroup.QueryFlag = 0
         Call QueryData(False)
         Call LoadLocation(Nothing, m_Locations, 1, "")
...
Private Sub InitFormLayout()
   ...
   Call InitNormalLabel(lblMotherNo, MapText("กลุ่มโรงเรือน"))
   Call InitNormalLabel(lblDocumentNo, MapText("รหัสกลุ่มโรงเรือน"))
